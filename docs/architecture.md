...
日期：2026-09-07。状态：方向已接受，组件和数据接口待最小验证后定版。

## 产品边界

通用的是任务入口、证据记录和知识管理；测试框架、业务正确性、设备和服务环境属于项目适配层。支持逐步接入异构项目，不承诺零配置自动完成所有测试。

首版采用本地 CLI 与单一协调进程。实现语言尚未锁定：先比较现有 Python SOP Runner 的可复用性和 TypeScript 方案的接入成本。协议保持语言无关。2026-09-07 本机只验证了 Python 3.12.3、Node.js v20.19.2 和 Git 2.43.0；这不是运行时支持矩阵，也不代表项目已通过环境验收。
